...
async function executeG1BehaviorEvaluation(job){
 ...
 for(const seed of seeds) for(const policy of policies){
  const rr=await runProcess({seconds,seed,policy,behaviorVersion:p.behaviorVersion||"1.0.0",objectMassKg:p.objectMassKg,objectSizeM:p.objectSizeM,targetDistanceM:p.targetDistanceM},"simulation/robot_behavior_worker.py");
  rr.robotId="unitree_g1";rr.behaviorId="pick-place";rr.engine="MuJoCo";rr.measured=true;rows.push(rr);
  await progress(job,rows.length,seeds.length*policies.length);
 }
 const mean=k=>rows.length?rows.reduce((a,r)=>a+Number(r[k]??0),0)/rows.length:0;
 const result={...(exp.result||{}),schemaVersion:"1.0",comparisonType:"robot-behavior-evaluation",robotId:"unitree_g1",behaviorId:"pick-place",behaviorVersion:p.behaviorVersion||"1.0.0",engine:"MuJoCo",measured:true,reproducible:true,task:"pick-place",taskConfig:{objectMassKg:p.objectMassKg??.22,objectSizeM:p.objectSizeM??.032,targetDistanceM:p.targetDistanceM??.22},runCount:rows.length,expectedRunCount:seeds.length*policies.length,successRate:rows.filter(r=>r.taskSuccess).length/Math.max(1,rows.length),summary:{taskSuccessRate:rows.filter(r=>r.taskSuccess).length/Math.max(1,rows.length),completionTimeMean:mean("completionTime"),controlCostMean:mean("controlCost"),maxTiltRadMean:mean("maxTiltRad"),collisionCountMean:mean("collisionCount")},rawResults:rows,validation:{passed:rows.length===seeds.length*policies.length,message:"Measured Unitree G1 pick-place evaluation completed."},provenance:{gitSha,worker:"humanoidbehavior-worker",execution:"g1-pick-place"}};
