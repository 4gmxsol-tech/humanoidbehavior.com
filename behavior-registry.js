...
async function countPrivate(userId){if(db.mode()==="postgresql"){const r=await db.query("SELECT COUNT(*)::int AS count FROM behaviors WHERE user_id=$1 AND visibility='private'",[userId]);return Number(r.rows[0]?.count||0)}return read().filter(x=>x.userId===userId&&x.visibility==="private").length}
...
module.exports={init,list,get,versions,save,publish,countPrivate};
