...
function manipulation(mj,behavior,seed,seconds,policy,version){
 ...
 for(let i=0;i<steps;i++){
  ...
  const kps=policy==="A"?36:30,kpe=policy==="A"?30:25,kds=policy==="A"?8:7,kde=policy==="A"?7:6;
  let ramp=Math.min(1,Number(data.time)/.8);ramp=ramp*ramp*(3-2*ramp);
  ...
  if(released){const ov=model.joint("object_free").dofadr[0],speed=Math.hypot(data.qvel[ov],data.qvel[ov+1],data.qvel[ov+2]);if(td<=.085&&speed<.08){if(settledSince==null)settledSince=Number(data.time);const req=version==="1.1.0"?.15:.05;if(Number(data.time)-settledSince>=req){success=true;complete=Number(data.time);break}}else settledSince=null}
 }
